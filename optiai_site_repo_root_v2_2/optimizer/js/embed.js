...
function App({root}){
  ...
  }; return [()=>s,(u)=>{s={...s,...(typeof u==='function'?u(s):u)}; render(html`<${App} root=${root} />`, root);}];})();
...
(async function mount(){
  ...
  if(dbg){
    try{
      ...
      if(res.ok){ dbg.textContent = "Functions: OK · Inline v" + VERSION; }
      else { dbg.textContent = "Functions: Not responding ("+res.status+") · Inline v" + VERSION; }
    }catch(e){
      dbg.textContent = "Functions: Not reachable · Inline v" + VERSION;
    }
  }
  if(!host) return;
  ...
  shadow.append(root);
  render(html`<${App} root=${root}/>`, root);
})();
